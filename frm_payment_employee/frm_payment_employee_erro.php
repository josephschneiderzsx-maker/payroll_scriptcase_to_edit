...
$old_error_handler = set_error_handler("frm_payment_employee_scriptcase_error_handler");

function frm_payment_employee_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['frm_payment_employee']['contr_erro']) && $_SESSION['scriptcase']['frm_payment_employee']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_frm_payment_employee->contr_frm_payment_employee->Apl_com_erro = true;
            if ($inicial_frm_payment_employee->contr_frm_payment_employee->NM_ajax_flag)
            {
                $inicial_frm_payment_employee->contr_frm_payment_employee->NM_ajax_info['errList']['geral_frm_payment_employee'][] = $msg;
                return;
            }
            frm_payment_employee_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function frm_payment_employee_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['frm_payment_employee'])  && '' != $_SESSION['scriptcase']['error_icon']['frm_payment_employee'])  ? $_SESSION['scriptcase']['error_icon']['frm_payment_employee']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['frm_payment_employee']) && '' != $_SESSION['scriptcase']['error_close']['frm_payment_employee']) ? $_SESSION['scriptcase']['error_close']['frm_payment_employee'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['frm_payment_employee']) && '' != $_SESSION['scriptcase']['error_span_title']['frm_payment_employee']) ? $_SESSION['scriptcase']['error_span_title']['frm_payment_employee'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['frm_payment_employee']) && '' != $_SESSION['scriptcase']['error_icon_title']['frm_payment_employee']) ? $_SESSION['scriptcase']['error_icon_title']['frm_payment_employee'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['frm_payment_employee']['error_buffer'] .= ob_get_flush();
}
