...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbl_batch_payroll']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbl_batch_payroll']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tbl_batch_payroll"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_Generate()
{
    ...
    scJs_confirm("<?php echo html_entity_decode('DO YOU WANT TO GENERATE THE PAYROLL?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Generate_ok, sc_btn_Generate_cancel)
}
...
var Crtl_btn_Historized = false;
function sc_btn_Historized()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Historized(); }, 500);
      return;
    }
    if (Crtl_btn_Historized) {return;}
    scJs_confirm("<?php echo html_entity_decode('DO YOU WANT TO HISTORIZED THE PAYROLL?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Historized_ok, sc_btn_Historized_cancel)
}
function sc_btn_Historized_cancel()
{
}
function sc_btn_Historized_ok()
{
    Crtl_btn_Historized = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Historized?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
...
function sc_btn_sc_btn_0()
{
    ...
    scJs_confirm("<?php echo html_entity_decode('DO YOU WANT TO RESET THE PAYROLL?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_sc_btn_0_ok, sc_btn_sc_btn_0_cancel)
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbl_batch_payroll']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_tbl_batch_payroll_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinigual*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_tbl_batch_payroll_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
