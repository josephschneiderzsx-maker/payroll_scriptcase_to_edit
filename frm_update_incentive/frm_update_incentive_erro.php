...
$old_error_handler = set_error_handler("frm_update_incentive_scriptcase_error_handler");

function frm_update_incentive_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['frm_update_incentive']['contr_erro']) && $_SESSION['scriptcase']['frm_update_incentive']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_frm_update_incentive->contr_frm_update_incentive->Apl_com_erro = true;
            if ($inicial_frm_update_incentive->contr_frm_update_incentive->NM_ajax_flag)
            {
                $inicial_frm_update_incentive->contr_frm_update_incentive->NM_ajax_info['errList']['geral_frm_update_incentive'][] = $msg;
                return;
            }
            frm_update_incentive_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function frm_update_incentive_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['frm_update_incentive'])  && '' != $_SESSION['scriptcase']['error_icon']['frm_update_incentive'])  ? $_SESSION['scriptcase']['error_icon']['frm_update_incentive']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['frm_update_incentive']) && '' != $_SESSION['scriptcase']['error_close']['frm_update_incentive']) ? $_SESSION['scriptcase']['error_close']['frm_update_incentive'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['frm_update_incentive']) && '' != $_SESSION['scriptcase']['error_span_title']['frm_update_incentive']) ? $_SESSION['scriptcase']['error_span_title']['frm_update_incentive'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['frm_update_incentive']) && '' != $_SESSION['scriptcase']['error_icon_title']['frm_update_incentive']) ? $_SESSION['scriptcase']['error_icon_title']['frm_update_incentive'] : '';
?>
