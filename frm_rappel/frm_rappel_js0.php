...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="frm_rappel"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['frm_rappel']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['frm_rappel']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['frm_rappel']['dashboard_info']['parent_widget'] . "']\")";
...
function sc_btn_MiscByDept()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_MiscByDept(); }, 500);
      return;
    }
    sc_btn_MiscByDept_ok();
}
function sc_btn_MiscByDept_cancel()
{
}
function sc_btn_MiscByDept_ok()
{
  tb_show('', '<?php echo $this->Ini->link_frm_UpdateRappelByDept_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=400&width=600&modal=true', '');
}
function sc_btn_MiscByValue()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_MiscByValue(); }, 500);
      return;
    }
    sc_btn_MiscByValue_ok();
}
function sc_btn_MiscByValue_cancel()
{
}
function sc_btn_MiscByValue_ok()
{
  tb_show('', '<?php echo $this->Ini->link_frm_UpdateRappelByValue_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=400&width=400&modal=true', '');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['frm_rappel']['where_detal']))
  {
?>  
...
function scForm_inline_submit(y, z) {
        Nm_Proc_Atualiz = true;
        do_ajax_frm_rappel_submit_form(y, z);
