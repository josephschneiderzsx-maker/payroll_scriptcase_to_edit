...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbl_batch_payroll_hist']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbl_batch_payroll_hist']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tbl_batch_payroll_hist"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
var Crtl_btn_Generate = false;
function sc_btn_Generate()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Generate(); }, 500);
      return;
    }
    if (Crtl_btn_Generate) {return;}
    scJs_confirm("<?php echo html_entity_decode('DO YOU WANT TO GENERATE THE HISTORIZED PAYROLL?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Generate_ok, sc_btn_Generate_cancel)
}
function sc_btn_Generate_cancel()
{
}
function sc_btn_Generate_ok()
{
    Crtl_btn_Generate = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Generate?@?";
    document.F1.action = "./";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbl_batch_payroll_hist']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_tbl_batch_payroll_hist_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinigual*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_tbl_batch_payroll_hist_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
   nm_field_disabled("payroll_code=disabled;status=disabled;payroll_description=disabled;date_debut=disabled;date_fin=disabled;date_debut_sem1=disabled;date_fin_sem1=disabled;date_debut_sem2=disabled;date_fin_sem2=disabled", "");
...
function scCssBlur(oHtmlObj)
{
 ...
 function nm_field_disabled(Fields, Opt) {
  ...
  for (i=0; i < Field.length; i++)
  {
     ...
     if (F_name == "payroll_code")
     {
        $('input[name="payroll_code"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="payroll_code"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="payroll_code"]').removeClass("scFormInputDisabled");
        }
     }
     if (F_name == "status")
     {
     ...
     if (F_name == "payroll_description")
     {
        $('textarea[name="payroll_description"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('textarea[name="payroll_description"]').addClass("scFormInputDisabled");
        }
        else {
            $('textarea[name="payroll_description"]').removeClass("scFormInputDisabled");
        }
     }
     if (F_name == "date_debut")
     {
        $('input[name="date_debut"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="date_debut"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="date_debut"]').removeClass("scFormInputDisabled");
        }
        $('input[id="calendar_date_debut"]').prop("disabled", F_opc);
        $("#id_sc_field_date_debut").datepicker("destroy");
        if (!F_opc) {
            scJQCalendarAdd("");
        }
     }
     if (F_name == "date_fin")
     {
        $('input[name="date_fin"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="date_fin"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="date_fin"]').removeClass("scFormInputDisabled");
        }
        $('input[id="calendar_date_fin"]').prop("disabled", F_opc);
        $("#id_sc_field_date_fin").datepicker("destroy");
        if (!F_opc) {
            scJQCalendarAdd("");
        }
     }
     if (F_name == "date_debut_sem1")
     {
        $('input[name="date_debut_sem1"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="date_debut_sem1"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="date_debut_sem1"]').removeClass("scFormInputDisabled");
        }
        $('input[id="calendar_date_debut_sem1"]').prop("disabled", F_opc);
        $("#id_sc_field_date_debut_sem1").datepicker("destroy");
        if (!F_opc) {
            scJQCalendarAdd("");
        }
     }
     if (F_name == "date_fin_sem1")
     {
        $('input[name="date_fin_sem1"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="date_fin_sem1"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="date_fin_sem1"]').removeClass("scFormInputDisabled");
        }
        $('input[id="calendar_date_fin_sem1"]').prop("disabled", F_opc);
        $("#id_sc_field_date_fin_sem1").datepicker("destroy");
        if (!F_opc) {
            scJQCalendarAdd("");
        }
     }
     if (F_name == "date_debut_sem2")
     {
        $('input[name="date_debut_sem2"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="date_debut_sem2"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="date_debut_sem2"]').removeClass("scFormInputDisabled");
        }
        $('input[id="calendar_date_debut_sem2"]').prop("disabled", F_opc);
        $("#id_sc_field_date_debut_sem2").datepicker("destroy");
        if (!F_opc) {
            scJQCalendarAdd("");
        }
     }
     if (F_name == "date_fin_sem2")
     {
        $('input[name="date_fin_sem2"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="date_fin_sem2"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="date_fin_sem2"]').removeClass("scFormInputDisabled");
        }
        $('input[id="calendar_date_fin_sem2"]').prop("disabled", F_opc);
        $("#id_sc_field_date_fin_sem2").datepicker("destroy");
        if (!F_opc) {
            scJQCalendarAdd("");
        }
     }
