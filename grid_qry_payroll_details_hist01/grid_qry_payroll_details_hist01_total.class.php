...
class grid_qry_payroll_details_hist01_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_qry_payroll_details_hist01']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_qry_payroll_details_hist01']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false, $ind_compara="", $tipo="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('dept' => "dept");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
          $cmps_quebra_atual[] = $cmp;
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['SC_force_fiels_gb'];
      }
      $Save_Format = '';
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['SC_clear_total']);
      $ind_cmps  = 42;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export, $Save_Format)
   {
     ...
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'] = array() ;  
      $nm_comando = "select count(*), sum(sal_brut_reg), sum(income_workday_ot), sum(income_holiday_ot), sum(income_restday_ot), sum(income_offday_ot), sum(incentive), sum(income_comm), sum(rappel), sum(tax_cass), sum(tax_cfgdct), sum(tax_fdu), sum(tax_iris), sum(tax_ona), sum(tax_iric), sum(dedeuct_cons), sum(loan_deduction), sum(loan_deduction_bank), sum(other_deduct), sum(total_deduct), sum(sal_net), count(distinct userid_int), count(userid_int), sum(Rate_SALF), sum(Rate_Work), sum(income_workday), sum(income_holiday), sum(income_restday), sum(income_offday), sum(hh_sunday), sum(sal_sunday), sum(hh_offday), sum(sal_holiday), sum(sal_leavetype), sum(ot_hh_w), sum(ot_hh_w_PM), sum(ot_hh_h), sum(ot_hh_h_PM), sum(ot_hh_r), sum(ot_hh_r_PM), sum(ot_hh_o), sum(ot_hh_o_PM) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['where_pesq']; 
      ...
      if ((isset($this->Ini->nm_bases_vfp) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_vfp)) || (isset($this->Ini->nm_bases_odbc) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_odbc)))
      {
          $vl_statistic = $this->Calc_statist_manual_sc_free_group_by($_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['where_pesq']);
          foreach ($vl_statistic as $ind => $val)
          {
              $rt->fields[$ind] = $val;
          }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][4] = $rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $rt->fields[4] = (string)$rt->fields[4]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][5] = $rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $rt->fields[5] = (string)$rt->fields[5]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][6] = $rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $rt->fields[6] = (string)$rt->fields[6]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][7] = $rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $rt->fields[7] = (string)$rt->fields[7]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][8] = $rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $rt->fields[8] = (string)$rt->fields[8]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][9] = $rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $rt->fields[9] = (string)$rt->fields[9]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][10] = $rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $rt->fields[10] = (string)$rt->fields[10]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][11] = $rt->fields[10]; 
      $rt->fields[11] = str_replace(",", ".", $rt->fields[11]);
      $rt->fields[11] = (string)$rt->fields[11]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][12] = $rt->fields[11]; 
      $rt->fields[12] = str_replace(",", ".", $rt->fields[12]);
      $rt->fields[12] = (string)$rt->fields[12]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][13] = $rt->fields[12]; 
      $rt->fields[13] = str_replace(",", ".", $rt->fields[13]);
      $rt->fields[13] = (string)$rt->fields[13]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][14] = $rt->fields[13]; 
      $rt->fields[14] = str_replace(",", ".", $rt->fields[14]);
      $rt->fields[14] = (string)$rt->fields[14]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][15] = $rt->fields[14]; 
      $rt->fields[15] = str_replace(",", ".", $rt->fields[15]);
      $rt->fields[15] = (string)$rt->fields[15]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][16] = $rt->fields[15]; 
      $rt->fields[16] = str_replace(",", ".", $rt->fields[16]);
      $rt->fields[16] = (string)$rt->fields[16]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][17] = $rt->fields[16]; 
      $rt->fields[17] = str_replace(",", ".", $rt->fields[17]);
      $rt->fields[17] = (string)$rt->fields[17]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][18] = $rt->fields[17]; 
      $rt->fields[18] = str_replace(",", ".", $rt->fields[18]);
      $rt->fields[18] = (string)$rt->fields[18]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][19] = $rt->fields[18]; 
      $rt->fields[19] = str_replace(",", ".", $rt->fields[19]);
      $rt->fields[19] = (string)$rt->fields[19]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][20] = $rt->fields[19]; 
      $rt->fields[20] = str_replace(",", ".", $rt->fields[20]);
      $rt->fields[20] = (string)$rt->fields[20]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][21] = $rt->fields[20]; 
      $rt->fields[21] = str_replace(",", ".", $rt->fields[21]);
      $rt->fields[21] = (string)$rt->fields[21]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][22] = $rt->fields[21]; 
      $rt->fields[22] = str_replace(",", ".", $rt->fields[22]);
      $rt->fields[22] = (string)$rt->fields[22]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][23] = $rt->fields[22]; 
      $rt->fields[23] = str_replace(",", ".", $rt->fields[23]);
      $rt->fields[23] = (string)$rt->fields[23]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][24] = $rt->fields[23]; 
      $rt->fields[24] = str_replace(",", ".", $rt->fields[24]);
      $rt->fields[24] = (string)$rt->fields[24]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][25] = $rt->fields[24]; 
      $rt->fields[25] = str_replace(",", ".", $rt->fields[25]);
      $rt->fields[25] = (string)$rt->fields[25]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][26] = $rt->fields[25]; 
      $rt->fields[26] = str_replace(",", ".", $rt->fields[26]);
      $rt->fields[26] = (string)$rt->fields[26]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][27] = $rt->fields[26]; 
      $rt->fields[27] = str_replace(",", ".", $rt->fields[27]);
      $rt->fields[27] = (string)$rt->fields[27]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][28] = $rt->fields[27]; 
      $rt->fields[28] = str_replace(",", ".", $rt->fields[28]);
      $rt->fields[28] = (string)$rt->fields[28]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][29] = $rt->fields[28]; 
      $rt->fields[29] = str_replace(",", ".", $rt->fields[29]);
      $rt->fields[29] = (string)$rt->fields[29]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][30] = $rt->fields[29]; 
      $rt->fields[30] = str_replace(",", ".", $rt->fields[30]);
      $rt->fields[30] = (string)$rt->fields[30]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][31] = $rt->fields[30]; 
      $rt->fields[31] = str_replace(",", ".", $rt->fields[31]);
      $rt->fields[31] = (string)$rt->fields[31]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][32] = $rt->fields[31]; 
      $rt->fields[32] = str_replace(",", ".", $rt->fields[32]);
      $rt->fields[32] = (string)$rt->fields[32]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][33] = $rt->fields[32]; 
      $rt->fields[33] = str_replace(",", ".", $rt->fields[33]);
      $rt->fields[33] = (string)$rt->fields[33]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][34] = $rt->fields[33]; 
      $rt->fields[34] = str_replace(",", ".", $rt->fields[34]);
      $rt->fields[34] = (string)$rt->fields[34]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][35] = $rt->fields[34]; 
      $rt->fields[35] = str_replace(",", ".", $rt->fields[35]);
      $rt->fields[35] = (string)$rt->fields[35]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][36] = $rt->fields[35]; 
      $rt->fields[36] = str_replace(",", ".", $rt->fields[36]);
      $rt->fields[36] = (string)$rt->fields[36]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][37] = $rt->fields[36]; 
      $rt->fields[37] = str_replace(",", ".", $rt->fields[37]);
      $rt->fields[37] = (string)$rt->fields[37]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][38] = $rt->fields[37]; 
      $rt->fields[38] = str_replace(",", ".", $rt->fields[38]);
      $rt->fields[38] = (string)$rt->fields[38]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][39] = $rt->fields[38]; 
      $rt->fields[39] = str_replace(",", ".", $rt->fields[39]);
      $rt->fields[39] = (string)$rt->fields[39]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][40] = $rt->fields[39]; 
      $rt->fields[40] = str_replace(",", ".", $rt->fields[40]);
      $rt->fields[40] = (string)$rt->fields[40]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][41] = $rt->fields[40]; 
      $rt->fields[41] = str_replace(",", ".", $rt->fields[41]);
      $rt->fields[41] = (string)$rt->fields[41]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['tot_geral'][42] = $rt->fields[41]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_dept_sc_free_group_by($dept, $Where_qb, $Cmp_Name) 
   {
      ...
      { 
         $where_ok = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_qry_payroll_details_hist01']['where_pesq'] . " and " . $Where_qb ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
