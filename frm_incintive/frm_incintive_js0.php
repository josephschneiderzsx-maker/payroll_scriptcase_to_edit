...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="frm_incintive"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['frm_incintive']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['frm_incintive']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['frm_incintive']['dashboard_info']['parent_widget'] . "']\")";
...
function sc_btn_RESET()
{
    ...
    scJs_confirm("<?php echo html_entity_decode('DO YOU WANT TO RESET EXTRA INCOME ?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_RESET_ok, sc_btn_RESET_cancel)
}
...
{
  tb_show('', '<?php echo $this->Ini->link_frm_update_incentive_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scoutsc_redir_atualiz*scinok*scoutsc_redir_insert*scinok*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=400&width=400&modal=true', '');
}
...
{
  tb_show('', '<?php echo $this->Ini->link_Frm_download_incentive_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=430&width=600&modal=true', '');
}
function sc_btn_IncentiveByDept()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_IncentiveByDept(); }, 500);
      return;
    }
    sc_btn_IncentiveByDept_ok();
}
function sc_btn_IncentiveByDept_cancel()
{
}
function sc_btn_IncentiveByDept_ok()
{
  tb_show('', '<?php echo $this->Ini->link_frm_UpdateIncentiveByDept_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=400&width=600&modal=true', '');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['frm_incintive']['where_detal']))
  {
?>  
...
function scForm_inline_submit(y, z) {
        Nm_Proc_Atualiz = true;
        do_ajax_frm_incintive_submit_form(y, z);
