...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="frm_loan_enterprise"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['frm_loan_enterprise']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['frm_loan_enterprise']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['frm_loan_enterprise']['dashboard_info']['parent_widget'] . "']\")";
...
?> 
var Crtl_btn_Button01 = false;
function sc_btn_Button01()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_Button01(); }, 500);
      return;
    }
    if (Crtl_btn_Button01) {return;}
    scJs_confirm("<?php echo html_entity_decode('DO YOU WANT TO APPLY LOAN TO STATEMENT?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Button01_ok, sc_btn_Button01_cancel)
}
function sc_btn_Button01_cancel()
{
}
function sc_btn_Button01_ok()
{
    Crtl_btn_Button01 = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Button01?@?";
    document.F1.action = "./";
...
{ 
   ...
   function nm_gp_print_conf(tp, cor, SC_module_export, password, ajax, str_type, bol_param)
   {
       ...
       {
           window.open('<?php echo$this->Ini->path_link . "" . SC_dir_app_name('frm_loan_enterprise') . "/frm_loan_enterprise_iframe_prt.php?path_botoes=" . $this->Ini->path_botoes . "&script_case_init=" . $this->form_encode_input($this->Ini->sc_page); ?>&apl_prt=frm_loan_enterprise.php&opcao=print&cor_print=' + cor,'','location=no,menubar,resizable,scrollbars,status=no,toolbar');
       }
   }
...
function scForm_inline_submit(y, z) {
        Nm_Proc_Atualiz = true;
        do_ajax_frm_loan_enterprise_submit_form(y, z);
