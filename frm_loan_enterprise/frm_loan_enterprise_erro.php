...
$old_error_handler = set_error_handler("frm_loan_enterprise_scriptcase_error_handler");

function frm_loan_enterprise_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['frm_loan_enterprise']['contr_erro']) && $_SESSION['scriptcase']['frm_loan_enterprise']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_frm_loan_enterprise->contr_frm_loan_enterprise->Apl_com_erro = true;
            if ($inicial_frm_loan_enterprise->contr_frm_loan_enterprise->NM_ajax_flag)
            {
                $inicial_frm_loan_enterprise->contr_frm_loan_enterprise->NM_ajax_info['errList']['geral_frm_loan_enterprise'][] = $msg;
                return;
            }
            frm_loan_enterprise_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function frm_loan_enterprise_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['frm_loan_enterprise'])  && '' != $_SESSION['scriptcase']['error_icon']['frm_loan_enterprise'])  ? $_SESSION['scriptcase']['error_icon']['frm_loan_enterprise']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['frm_loan_enterprise']) && '' != $_SESSION['scriptcase']['error_close']['frm_loan_enterprise']) ? $_SESSION['scriptcase']['error_close']['frm_loan_enterprise'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['frm_loan_enterprise']) && '' != $_SESSION['scriptcase']['error_span_title']['frm_loan_enterprise']) ? $_SESSION['scriptcase']['error_span_title']['frm_loan_enterprise'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['frm_loan_enterprise']) && '' != $_SESSION['scriptcase']['error_icon_title']['frm_loan_enterprise']) ? $_SESSION['scriptcase']['error_icon_title']['frm_loan_enterprise'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['frm_loan_enterprise']['error_buffer'] .= ob_get_flush();
}
