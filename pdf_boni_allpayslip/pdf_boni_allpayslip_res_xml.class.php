...
class pdf_boni_allpayslip_res_xml
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function monta_xml()
   {
      ...
      $this->grava_arquivo();
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['pdf_boni_allpayslip']['xml_res_grid']))
      {
          return;
      }
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['pdf_boni_allpayslip']['xml_res_grid']) && !$this->Ini->sc_export_ajax) {
   ...
   function grava_arquivo()
   {
      ...
      if ($this->New_Format)
      {
          fwrite($xml_f, "<pdf_boni_allpayslip>\r\n");
      }
      if ($this->Grava_view)
      {
          ...
          if ($this->New_Format)
          {
              fwrite($xml_v, "<pdf_boni_allpayslip>\r\n");
          }
      }
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['pdf_boni_allpayslip']['xml_res_grid']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['pdf_boni_allpayslip']['xml_res_file']['xml'] = $this->Xml_f;
          if ($this->Grava_view)
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['pdf_boni_allpayslip']['xml_res_file']['view'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo_view;
          }
      }
   ...
   function grava_linha($xml_f)
   {
      ...
      foreach ($this->array_linhas as $lines)
      {
          ...
          if (!$this->New_Format)
          {
              $xml_registro = "<pdf_boni_allpayslip";
          }
          foreach ($lines as $columns)
          {
              if (0 <= $columns['level'])
