...
$old_error_handler = set_error_handler("form_employee_setting_scriptcase_error_handler");

function form_employee_setting_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['form_employee_setting']['contr_erro']) && $_SESSION['scriptcase']['form_employee_setting']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_employee_setting->contr_form_employee_setting->Apl_com_erro = true;
            if ($inicial_form_employee_setting->contr_form_employee_setting->NM_ajax_flag)
            {
                $inicial_form_employee_setting->contr_form_employee_setting->NM_ajax_info['errList']['geral_form_employee_setting'][] = $msg;
                return;
            }
            form_employee_setting_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_employee_setting_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_employee_setting'])  && '' != $_SESSION['scriptcase']['error_icon']['form_employee_setting'])  ? $_SESSION['scriptcase']['error_icon']['form_employee_setting']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_employee_setting']) && '' != $_SESSION['scriptcase']['error_close']['form_employee_setting']) ? $_SESSION['scriptcase']['error_close']['form_employee_setting'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_employee_setting']) && '' != $_SESSION['scriptcase']['error_span_title']['form_employee_setting']) ? $_SESSION['scriptcase']['error_span_title']['form_employee_setting'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_employee_setting']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_employee_setting']) ? $_SESSION['scriptcase']['error_icon_title']['form_employee_setting'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_employee_setting']['error_buffer'] .= ob_get_flush();
}
