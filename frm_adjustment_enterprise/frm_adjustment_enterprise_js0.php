...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="frm_adjustment_enterprise"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['frm_adjustment_enterprise']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['frm_adjustment_enterprise']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['frm_adjustment_enterprise']['dashboard_info']['parent_widget'] . "']\")";
...
function sc_btn_sc_btn_0()
{
    ...
    scJs_confirm("<?php echo html_entity_decode('DO YOU WANT TO APPLY ADJUSTMENTS TO STATEMENTS?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_sc_btn_0_ok, sc_btn_sc_btn_0_cancel)
}
...
{ 
   ...
   function nm_gp_print_conf(tp, cor, SC_module_export, password, ajax, str_type, bol_param)
   {
       ...
       {
           window.open('<?php echo$this->Ini->path_link . "" . SC_dir_app_name('frm_adjustment_enterprise') . "/frm_adjustment_enterprise_iframe_prt.php?path_botoes=" . $this->Ini->path_botoes . "&script_case_init=" . $this->form_encode_input($this->Ini->sc_page); ?>&apl_prt=frm_adjustment_enterprise.php&opcao=print&cor_print=' + cor,'','location=no,menubar,resizable,scrollbars,status=no,toolbar');
       }
   }
...
function scForm_inline_submit(y, z) {
        Nm_Proc_Atualiz = true;
        do_ajax_frm_adjustment_enterprise_submit_form(y, z);
